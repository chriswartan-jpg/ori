> **Nicht die gebaute Architektur.** Dieses Dokument beschreibt ein Produktziel, das
> `linkedin-mcp-server` (Browser-Automation gegen linkedin.com) einsetzt und damit
> `docs/PROJECT.md` Key Decisions, `AGENTS.md` §Forbidden und `ori-project-idea.md` §5
> widerspricht — siehe §11 dieses Dokuments, Risiko 1. Der MVP ist nach
> `docs/ARCHITECTURE.md` gebaut (Export-only, Draft-only). Der Widerspruch ist eine
> offene Team-Entscheidung, siehe `docs/TASKS.md`.
